...
A probability bound on the probability of resampling all zeros.  That is, if a feature has number of zeros greater than $n \ast \text{errthresh}^{1/n} $  , that feature is excluded
...
my.test.sim.score <- function(x,y=NULL,constant=0.5){
    return(constant + cor(x,y))
   }
