...
\text{where $C_{ij}$ is the number of generalized checkerboard scores when comparing species $i$ and $j$ and $m$ is the total number of species in the community.}
\]

\noindent
and finally normalizes this between -1 and 1 for $n$ bins based on the expected ratio of co-variation to co-exclusion patterns in any given sample.  The adjustment factor is given by:
